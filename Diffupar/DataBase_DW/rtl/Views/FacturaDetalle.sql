CREATE VIEW rtl.FacturaDetalle
as
select 
L.GL_DATEPIECE,
CONVERT(TIME, l.GL_DATECREATION) as 'tiempo',
l.GL_NUMERO NroDoc,--- as nro_doc_cegid,
l.GL_NUMLIGNE NroLinea,--- as nro_linea,
l.GL_TYPEARTICLE TipoArticulo,---- as tipo_articulo,
l.GL_QTEFACT CantLineas,---- as cant_linea,
l.GL_CODEARTICLE ,
...
l.GL_TOTALTTC Total_Civa,--- as 'total_IVA_inc_linea',

L.GL_QTERESTE CantidadItems,
l.GL_COLLECTION Incentivo,---- as 'COD. INCENTIVO',

A.GA_LIBELLE AS article_desc,
CONCAT( l.GL_ETABLISSEMENT,l.GL_NUMERO) as fk_piece,
...
-----CONCAT( cast (CH1.YX_CODE as int),cast(T.T_TIERS as int), cast(FN4.cc_code as int))as pk_cost,
...
from [200.32.54.90].Produccion.[dbo].[LIGNE] AS l WITH (NoLock)
inner JOIN [200.32.54.90].Produccion.[dbo].ARTICLE AS A  WITH (NoLock)ON A.GA_CODEARTICLE=L.GL_CODEARTICLE
inner join [200.32.54.90].Produccion.[dbo].ARTICLECOMPL as ac WITH (NoLock) on ac.GA2_ARTICLE=A.GA_ARTICLE
LEFT JOIN
(SELECT 
T_TIERS,
T_LIBELLE,
...
from [200.32.54.90].Produccion.dbo.CHOIXEXT WITH (NoLock)
